---
layout: page
title: about me
permalink: /about/
nav: true
nav_order: 1
show_title: false
---

<style>

.post{

    padding-top:0.5cm;

    padding-bottom:1.5cm;

}

h2{

    margin-top:1.5cm;

    margin-bottom:0.5cm;

}

h2:first-of-type{

    margin-top:0cm;

}

</style>



## PhD at IIT Bombay

I joined the PhD programme at the Centre for Machine Intelligence and Data Science (CMInDS), IIT Bombay in January 2025. I am advised by [Prof. Usha Ananthakumar](https://www.som.iitb.ac.in/prof-ms-usha-ananthakumar/) and co-advised by [Dr. Piyush Pandey](https://www.som.iitb.ac.in/dr-piyush-pandey/), both at the Shailesh J. Mehta School of Management.

My broad research interest is in understanding how machine learning methods — in particular graph neural networks and statistical learning approaches — can model complex dependency structures in financial systems. Credit risk is a natural domain for this: borrowers, institutions, and instruments are interconnected, and these relationships carry information that flat tabular models do not capture well.


## PhD Coursework

### Spring 2024–25

| Course Code | Course |
|-------------|---------|
| IE 614 | Linear Systems |
| SC 607 | Optimization |
| SI 509 | Time Series Analysis |
| GC 101 | Gender in the Workplace |
| TA 101 | Teaching Assistant Skill Enhancement & Training (TASET) |

---

### Autumn 2025–26

| Course Code | Course |
|-------------|---------|
| CS 725 | Foundations of Machine Learning |
| EE 601 | Statistical Signal Analysis |
| DSS801 | Seminar |

---

### Spring 2025–26

| Course Code | Course |
|-------------|---------|
| CS6106 | Statistical Learning Theory |
| EE6106 | Online Learning and Optimisation |
| DS899 | Communication Skills |

## Teaching Experience

| Semester | Course | Role |
|-----------|---------|------|
| Spring 2025–26 | Machine Learning for Finance | Head Teaching Assistant |
| Autumn 2025–26 | Statistical Foundations of Machine Learning | Teaching Assistant |



## Academic recognition

I qualified the GATE Statistics examination (2024) and IIT JAM Mathematical Statistics (2024), both among the top 5% of candidates nationally.

## Workshops and schools

I have participated in several workshops and academic schools that have shaped my research perspective:


| Year | Event | Venue |
|------|-------|-------|
| Dec 2025 | [ACM Winter School](https://alphagrep.iiit.ac.in/acm-winter-school.html) | IIIT Hyderabad |
| Jul 2025 | Joint Telematics Group Summer School on Signal Processing, Communications and Networks | IIT Bombay |
| Jan 2025 | Statistical Methods in Finance 2025 *(oral presentation)* | Chennai Mathematical Institute |
| Jan 2018 | Winter School on Mathematics | ISI North East Centre, Tezpur |
